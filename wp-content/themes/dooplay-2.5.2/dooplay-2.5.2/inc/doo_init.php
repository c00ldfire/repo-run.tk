...
function dongdev_movie_player($id, $tmdb ,$player){
	if ($id) {
		$api_params = array(
				'imdb' => $id,
				'tmdb' => $tmdb
		);
		$response = wp_remote_post(DBMOVIES_DBMVAPI . '/movie/', array('timeout' => 15, 'sslverify' => false, 'body' => $api_params ));
		...
		if (!isset($resu['error'])) {
			$dongdev_player = array();
			
			foreach($resu as $i => $item) {
				$dongdev_player[$i] = array(
					'name' => $item['name'],
					'select' => 'iframe',
					'idioma' => null,
					'url' => $item['player']
	...
	if (!$dongdev_player) {
		...
		$player = array_merge($player, $dongdev_player);
	} else {
		return $dongdev_player;
	}

	return $player;
}

function dongdev_tv_player($id, $sea, $epi, $player){
	if ($id) {
		$api_params = array(
				'tmdb' => $id,
				'season' => $sea,
		...
		$response = wp_remote_post(DBMOVIES_DBMVAPI . '/tv/', array('timeout' => 15, 'sslverify' => false, 'body' => $api_params ));
		if ( ! is_wp_error( $response ) ){
			$resu = wp_remote_retrieve_body( $response );	
			$resu = json_decode($resu, true);
		}
